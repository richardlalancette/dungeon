...
%---------------------------------------------------------------------------
% Trouve sur l'Internet, Modifie par Richard Lalancette(A la fin)
% Module pour lire les donnees fournies par une manette de Xbox 360 par port USB
module joystick
    export GetInfo, isPlugged,
    ...
    procedure GetInfo (jNum : int, var btnpressed : array 1 .. * of boolean)
        %Edited from original turing module
    ...
    %---------------------------------------------------------------------------
    % Richard Lalancette
    % This forces a read of the joystick to see if plugged.
    % we could potentially check that in the GetInfo instead?
    function isPlugged( jNum : int ) : boolean
        var caps : CapsRecord
        
        Capabilities (jNum, caps)
